    ...
    if len(last_purchases) >= 1:
        ...
        for purchases in last_purchases:
            save_purchases.append(f"<b>📃 Чек:</b> <code>#{purchases[4]}</code>\n"
                                  f"▶ {purchases[9]} | {purchases[5]}шт | {purchases[6]}грн\n"
                                  f"🕜 {purchases[13]}\n"
...
async def open_category_for_create_position(call: CallbackQuery, state: FSMContext):
    ...
    send_msg = f"<b>📥 Покупка товара:</b>\n" \
               ...
               f"<b>💵 Стоимость:</b> <code>{get_position[3]}грн</code>\n" \
    ...
    if len(get_items) >= 1:
        if int(get_user[4]) >= int(get_position[3]):
            ...
            await call.message.answer(f"📦 <b>Введите количество товаров для покупки</b>\n"
                                      ...
                                      f"💵 Стоимость товара: <code>{get_position[3]}грн</code>\n"
                                      f"💳 Ваш баланс: <code>{get_user[4]}грн</code>\n",
        ...
        if len(get_items) >= 1:
            if 1 <= get_count <= len(get_items):
                if int(get_user[4]) >= amount_pay:
                    await state.finish()
                    delete_msg = await message.answer("<b>⚡️ Товары подготовлены.</b>",
                                                      reply_markup=check_user_out_func(message.from_user.id))

                    await message.answer(f"<b>⚡️ Вы действительно хотите купить товар(ы)?</b>\n"
                                         f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                         f"🏷 Название товара: <code>{get_position[2]}</code>\n"
                                         f"💵 Стоимость товара: <code>{get_position[3]}грн</code>\n"
                                         f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                         f"▶ Количество товаров: <code>{get_count}шт</code>\n"
                                         f"💰 Сумма к покупке: <code>{amount_pay}грн</code>",
...
async def yes_buy_this_item(call: CallbackQuery, state: FSMContext):
    ...
    if 1 <= int(get_count) <= len(get_items):
        if int(get_user[4]) >= amount_pay:
            ...
            await call.message.answer(f"<b>⚡️ Вы успешно купили товар(ы) ✅</b>\n"
                                      f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                      f"📃 Чек: <code>#{receipt}</code>\n"
                                      f"🏷 Название товара: <code>{get_position[2]}</code>\n"
                                      f"📦 Куплено товаров: <code>{get_count}</code>\n"
                                      f"💵 Сумма покупки: <code>{amount_pay}грн</code>\n"
        ...
        await state.finish()
        await call.message.answer("<b>❌ Товар который вы хотели купить закончился.</b>",
