...
async def send_work_message(message: types.Message, state: FSMContext):
    if "id" in message:
        await message.answer("🔴 Бот находится на технических работах Для покупки товара пишите 👨🏻‍💻 @torchki_support_bot..")
    else:
        await message.answer("<b>🔴 Бот находится на технических работах Для покупки товара пишите 👨🏻‍💻 @torchki_support_bot.</b>")
...
async def bot_start(message: types.Message, state: FSMContext):
    ...
    await message.answer_photo(photo=data, caption=f'<b>👋🏻 Привет, <code>{message.from_user.first_name}</code>!\n\n'
                                                   f'⚡️Добро пожаловать в самый лучший и отзывчивый магазин Torchki⚡️Shop.</b>\n\n',reply_markup=check_user_out_func(message.from_user.id))
...
async def send_user_message(message, state: FSMContext):
    if "id" in message:
        await message.answer("🔴 Покупки в боте временно Не работают Для покупки товара пишите 👨🏻‍💻 @torchki_support_bot", True)
    else:
        await message.answer("<b>🔴 Покупки в боте временно Не работают  Для покупки товара пишите 👨🏻‍💻 @torchki_support_bot</b>")
